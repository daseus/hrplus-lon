...
test("parseNumber hanterar även internationellt format", () => {
  ...
  assert.equal(parseNumber("1234.56"), 1234.56); // engelskt utan tusentalsavgränsare
...
test("categorizeRow klassificerar lönerader", () => {
  assert.equal(categorizeRow("990", "Arbetsgivaravgift", "accounting"), "technical");
...
test("categorizeRow döljer tekniska påslag endast i bokföringsposter", () => {
  assert.equal(categorizeRow("872", "Förmån bostad pensionsgr", "payrollDraftI"), "other");
  assert.equal(categorizeRow("872", "Förmån bostad pensionsgr", "payrollDraftHr"), "other");
  assert.equal(categorizeRow("872", "Förmån bostad pensionsgr", "accounting"), "technical");
});
...
test("getSourceType beskriver I:-underlaget som färdigt och komplett", () => {
  const source = getSourceType(false, false, false, "payrollDraftI");
  assert.match(source.sourceDescription, /färdiga och kompletta/i);
  assert.doesNotMatch(source.sourceDescription, /preliminär/i);
});
