...
function normalizeSingleSeparator(value, separator) {
  const signless = value.replace(/^[+-]/, "");
  const parts = signless.split(separator);
  const decimalDigits = parts.at(-1).length;
  const isGroupedThousands = decimalDigits === 3
    && parts.slice(1).every((part) => part.length === 3)
    && parts[0].length <= 3;
  if (isGroupedThousands) return value.split(separator).join("");

  const lastIndex = value.lastIndexOf(separator);
  return value.slice(0, lastIndex).split(separator).join("")
    + "."
